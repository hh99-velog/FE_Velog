...
const Detail = (props) => {
  ...
  const deleteBtn = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("삭제되었습니다", "더 좋은 글을 올려주세요 :)", "success");
        history.push("/");
      }
    });
  };
  ...
  return (
    <DetailStyle margin="0 auto" padding="40px 0">
      ...
      <Grid is_flex>
        <Grid is_flex>
          <Text width="auto" bold>
            by {`${list.nickname}`}
          </Text>
          <Text width="auto" margin="0 auto 0 10px" size="14px">
            {list.createdAt}
          </Text>
        </Grid>
        {token ? (
      ...
      {/* Comment Add */}
      <AddComments
        cmtCount={commentList.length}
        board_id={boardId[2]}
      ></AddComments>
      {/* Comment List */}
      {commentList.map((cd, idx) => {
        return <CommentList board_id={boardId[2]} cd={cd}></CommentList>;
      })}
    </DetailStyle>
  );
};
